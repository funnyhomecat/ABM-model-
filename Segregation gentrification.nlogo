    ...
    if random 100 < density [   ; set the occupancy density
      sprout 1 [
        ...
        set size 1
          ...
          if color = 105 and random-float 1 <= 0.3 [
            set bobo? true
            set shape "bug"
          ]
...
to update-turtles
  ask turtles [
    ...
    ifelse bobo? = true [
      ;If turtle is a bobo, behave like an orange turtle
      let similar-orange count (turtles-on neighbors) with [color = 27]
      let other-orange count (turtles-on neighbors) with [color != 27]
      let total-orange similar-orange + other-orange
      set happy? similar-orange >= (%-similar-wanted * total-orange / 100)
    ]
    [
      ...
      ask turtles with [bobo? = true] [set shape "bug"]
    ]
  ]
end
