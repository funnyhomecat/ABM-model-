...
;; This is an extension to the two-person iterated prisoner's dilemma. I customized the
;custom strategy to include a memory of the previous five rounds. The custom strategy now
;cooperates ONLY if there are less than two defections from the partner in the previous five
;rounds of play. Defection history starts off in a new game with 2 defections and 3 cooperations.
;When I built the model i randomized starting defections and cooperations, before i realized
;a specific starting point was asked for.

;As for the outcomes as between the computer playing tit-for-tat or tit-fot-two-tat respectively,
;it does not seem to matter with our starting defection history. This makes sense as we start with
;two defections, meaning we will defect the first round. In tit-for-tat play, the computer will now
;defect as a response.
;playing tit-for-two-tat, we will defect the first round, and the computer will cooperate. In the
;second round, the computer will cooperate again and we will defect. This triggers the computer
;to start defecting, and we will answer by defecting - ending up in endless defection.
;when i had a randomized starting history, this did not always happen, presumably because we started
;the defection history with one or less defections.
...
turtles-own [
  ...
  defection-history      ; initialize new var. Did my partner defect last five rounds?
]
...
to setup
  ...
  ask turtles [
    ...
    set defection-history (list 0 0 1 1 1) ; Set starting history of defections
    set score 3.0
  ]
...
to custom-strategy ; Turtle Procedure
  ifelse sum defection-history >= 2 ; If the current number of defections in the last five rounds is > 1, defect, else cooperate
    [ set defect-now? true ]
    [ set defect-now? false ]
end
...
to get-payoff ; Turtle Procedure
  ...
  set defection-history fput ifelse-value partner-defected? = true [1] [0] defection-history ; If partner defected, add 1 to defection history. Else, add 0
  set defection-history but-last defection-history ; remove last item from defection history, i.e. the now 6th item to keep the memory 5 rounds long
  print defection-history
  ifelse partner-defected?
    [ ifelse defect-now?
      [ set score score + 1 ]
      [ set score score + 0 ]
